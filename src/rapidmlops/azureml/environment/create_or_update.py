...
def create_or_update_environment(
    ...
    """Creates or updates a environment in Azure Machine Learning workspace.

    Args:
        environment_config_path (str): The path to the environment configuration.
        name (str, optional): The Environment name, default is None (use the name from the environment config file).
    ...
    logger.info(
        f"[INFO] Loading Environment configuration from: {environment_config_path}"
    )
    ...
    logger.info(
        f"[INFO] Environment {environment.name}:{environment.version} created/updated successfully "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create or update a environment in Azure Machine Learning workspace"
    )
    parser.add_argument(
        "--environment_config_path",
        type=str,
        required=True,
        help="The path to the environment configuration.",
    )
    parser.add_argument(
        "--name", type=str, help="The Environment name (optional override)."
    )
    parser.add_argument(
        "--commit_sha",
        type=str,
        help="The Git Commit SHA (determines Environment Version).",
    )
    ...
    create_or_update_environment(
        environment_config_path=args.environment_config_path,
        name=args.name,
